...
def validate_coupon():
    ...
    if not coupon:
        return jsonify({'valid': False, 'message': 'Invalid or Inactive Coupon Code.'})
        
    if datetime.strptime(coupon['expiry_date'], '%Y-%m-%d') < datetime.now():
        return jsonify({'valid': False, 'message': 'This coupon has expired.'})
        
    if coupon.get('current_uses', 0) >= coupon.get('max_uses', 999):
    ...
    if coupon['type'] == 'percentage':
        ...
        if discount > coupon.get('max_discount', 0):
            discount = coupon['max_discount']
    else:
        discount = coupon['value']
        
    final_amount = amount - discount
    return jsonify({'valid': True, 'discount': discount, 'final_amount': final_amount, 'message': f'₹{discount} Discount Applied!'})
...
def create_order():
    ...
    # Check duplicate user before creating order
    ...
    try:
        ...
        if payment['status'] == 'captured' or payment['method'] == 'upi':
            ...
            transaction = {
                ...
                'order_id': razorpay_order_id, 'payment_id': razorpay_payment_id, 'transaction_id': razorpay_payment_id,
            ...
            if user.get('coupon_code'):
                db.coupons.update_one({'code': user['coupon_code']}, {'$inc': {'current_uses': 1}})
...
def download_receipt(user_id):
    ...
    data = [
        ...
        ("Registration ID", str(user['_id'])[-8:]),
        ("Membership Plan", user.get('plan', '')),
        ("Original Amount", f"Rs. {user.get('amount', 0)}"),
...
def services():
    services_list = [
        ...
        {'icon': '🥗', 'title': 'Nutritional Guidance', 'desc': 'Custom diet plans tailored to your goals.', 'img': 'https://images.unsplash.com/photo-1490645935967-10de6ba17061?w=800&q=80', 'benefits': ['Personalized Diet', 'Fat Loss Support', 'Muscle Gain Diet', 'Lifestyle Management']},
