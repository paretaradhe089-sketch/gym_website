...
def register_cash():
    ...
    # CASH PAYMENT TRACKING FIX (Taaki Dashboard par dikhe)
    payment_record = {
        'user_id': user_id,
        'name': name,
        'phone': phone,
        ...
        'order_id': 'CASH',
        'payment_id': 'N/A',
        'transaction_id': f'CASH-{user_id[-6:]}',
        'payment_method': 'Cash',
        'status': 'SUCCESS',
        'date': join_date.strftime('%d %b %Y'),
        'time': join_date.strftime('%I:%M %p'),
        'receipt': f"CASH-{user_id[-6:]}"
    }
    ...
    try:
        ...
        if payment['status'] == 'captured' or payment['method'] == 'upi':
            ...
            transaction = {
                ...
                'payment_method': payment['method'], 'upi_app': get_upi_app(payment.get('vpa', '')), 'vpa': payment.get('vpa', 'N/A'),
                'status': 'SUCCESS',
                'date': ist_now.strftime('%d %b %Y'), 'time': ist_now.strftime('%I:%M %p'),
                'receipt': f"SFZ{str(user_id)[-6:]}"
            }
            db.payments.insert_one(transaction)
...
def download_receipt(user_id):
    ...
    pdf.cell(70, 4, f"Invoice No: {invoice_no}", 0, 1, 'R')
    pdf.set_xy(130, 24)
    pdf.cell(70, 4, f"Date: {datetime.now(IST).strftime('%d %b %Y %I:%M %p')}", 0, 1, 'R')
    pdf.set_xy(130, 28)
    pdf.set_text_color(46, 204, 113)
    pdf.set_font("Arial", 'B', 8)
    pdf.cell(70, 4, "Status: SUCCESS", 0, 1, 'R')
...
def contact():
    ...
    if request.method == 'POST':
        ...
        if not name or not message:
            flash('⚠️ Naam aur Message zaroori hai!', 'error')
            return redirect(url_for('main.contact'))
        db.feedback.insert_one({'name': name, 'email': request.form.get('email'), 'message': message, 'created_at': datetime.now(IST), 'is_approved': False})
